...
| `allowed-tools` | No | Single space-separated string (not a YAML sequence) of pre-approved tools the skill may use. (Experimental) |
...
- A single YAML scalar string of space-separated tool names that are pre-approved to run
- Not a YAML sequence. Do not use list syntax (`[...]` or `- item`); a parser expecting a string will reject it
...
**Invalid example:**
```yaml
# YAML sequence syntax is not valid for this field
allowed-tools:
  - Bash(git:*)
  - Read
```
