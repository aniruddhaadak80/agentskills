...
The core pattern is to run each test case in two configurations: **with the skill** and **without it** (or with a previous version). The second configuration gives you a baseline to compare against. Run each configuration more than once — three runs is a practical minimum — and store every run in its own `run-N/` subdirectory, so you can report a mean and a standard deviation instead of a single number.
...
└── evals/
    ├── evals.json
    └── files/             # Input files referenced by the "files" field
...
└── iteration-1/
    ...
    │       ├── run-2/
    │       └── run-3/
    ├── eval-clean-missing-emails/   # Same structure as eval-top-months-chart
    └── benchmark.json         # Aggregated statistics across all runs
```

The main file you author by hand is `evals/evals.json`. The other JSON files (`grading.json`, `timing.json`, `benchmark.json`) are produced during the eval process — by the agent, by scripts, or by you. Each `run-N/` directory holds the artifacts for a single execution.
...
- Save outputs to: csv-analyzer-workspace/iteration-1/eval-top-months-chart/with_skill/run-1/outputs/
```

For the baseline, use the same prompt but without the skill path, saving to `without_skill/run-1/outputs/`. Repeat the same instructions for `run-2/` and `run-3/`.

When improving an existing skill, use the previous version as your baseline. Snapshot it before editing (`cp -r <skill-path> <workspace>/skill-snapshot/`), point the baseline run at the snapshot, and save to `old_skill/run-1/outputs/` instead of `without_skill/`.
...
Once every run in the iteration is graded, compute summary statistics per configuration and save them to `benchmark.json` alongside the eval directories (e.g., `csv-analyzer-workspace/iteration-1/benchmark.json`). Each `mean` covers every run of every eval in that configuration, `tokens` is the mean of each run's `total_tokens`, `time_seconds` is the mean of each run's `duration_ms`, and `stddev` is the population standard deviation of those same values:

```json benchmark.json
{
  "run_summary": {
    "with_skill": {
      "pass_rate": { "mean": 0.88, "stddev": 0.13 },
      "time_seconds": { "mean": 45.0, "stddev": 9.8 },
      "tokens": { "mean": 86000, "stddev": 800 }
    },
    "without_skill": {
      "pass_rate": { "mean": 0.38, "stddev": 0.13 },
      "time_seconds": { "mean": 32.0, "stddev": 4.9 },
      "tokens": { "mean": 24000, "stddev": 800 }
    },
    "delta": {
      "pass_rate": 0.50,
      "time_seconds": 13.0,
...
<Note>
Standard deviation (`stddev`) is only meaningful with multiple runs per configuration. If an early iteration gives you a single run per configuration, leave `stddev` out and focus on the raw pass counts and the delta — the spread becomes useful once you run each configuration repeatedly.
</Note>
